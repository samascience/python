...
for row in reader:
    ...
    tn.read_until(">",5)
    tn.write("show lldp neighbors\r")
    out = tn.read_until(">")
    out = out.split("\n")
    ...
    for i in range(0,len(fan_list)):
        ...
        str1 = str(fan_list[i][0]+'\W+'+fan_list[i][1]+'\W+'+fan_list[i][2]+'\W+'+fan_list[i][3]+'\W+'+fan_list[i][4])
        #str1 = str(fan_list[i][0]+'\W+'+fan_list[i][1]+'\W+'+fan_list[i][2]+'\W+'+fan_list[i][3])
        if (re.match(out[i+3],str1)!= None):
